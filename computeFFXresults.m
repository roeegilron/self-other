function computeFFXresults(subsToExtract,fold,computeStzler,ffxResFold,outDir,numMaps,slsize,settings,params)
...
for i = subsToExtract
    start = tic;
    subStrSrc = sprintf('*shuf*sub_%d*.mat',i);
    ff = findFilesBVQX(ffxResFold,subStrSrc);
    if ~isempty(ff) % make sure this subject exists 
...
if settings.incomsubsflag
    modesubs = 'allsubs';
else
    modesubs = 'partial_subs';
end
[pn,fn]= fileparts(ffxResFold);
if computeStzler
    fnTosave = sprintf(...
        'ND_FFX_%d-subs_%d-slsze_%d-fld_%dshufs_%d-stlzer_mode-%s_%s_newT2013.mat',...
        numsubs,...
        slsize,...
        fold,...
        numshufs,...
    ...
    fnTosave = sprintf(...
        'ND_FFX_%d-subs_%d-slsze_%d-fld_%dshufs_mode-%s_%s_newT2013_SVM.mat',...
        numsubs,...
        slsize,...
        fold,...
        numMaps,...
