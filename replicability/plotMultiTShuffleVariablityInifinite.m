function plotMultiTShuffleVariablityInifinite()
...
if ~skip
    for r = rois
        ff = findFilesBVQX(rootdir,sprintf(filepat,r));
        if ~isempty(ff)
            for f = 1:size(ff,1) % loop on shuffels
                load(ff{f});
                datashufd(:,f) = dataprev(:,1); % only multi vals
                percout(f) = perc;
            end
            hfig = figure;
            ...
            var(datashufd,1)
            boxplot(datashufd')
            title('Variance in Shuffels Within Each Subject');
            xlabel('subject #');
            ylabel('Multi T Variance Within Subjects');
            % find real data
            realfn = sprintf('roi_%0.3d_inf_prev.mat',r);
            ffreal = findFilesBVQX(rootdir,sprintf(realfn,r));
            load(ffreal{1});
            % real data 80 trials
            hold on;
            scatter(1:size(datashufd,1),...
                dataprev(dataprev(:,2)==80,1),...
                40,[1 0 0],'o','filled','MarkerFaceAlpha',0.5);
            %% make plot of shuf data per subject
            subplot(2,2,2)
            boxplot(datashufd);
            hbox = gca;
            ...
            title('Variance Between Subjects Across Shuffels');
            xlabel('shuffle #');
            ylabel('Multi T Variance Between Subjects');
            % make manhattan plot
            % compute pvals:
            alldat = [dataprev(dataprev(:,2)==80,1), datashufd];
            pvals = calcPvalVoxelWise(alldat) ;
