function plotMultiTShuffleVariablityInifinite_cross_validated()
...
rootdir = fullfile('..','..','results','Replicability','ss_infinite_prevelance_v-cross_validate');
figdir = fullfile('..','..','figures','replicability','infinte_variability_shufs_cross-validate');
...
% rois = 60;
for r = rois
    fnmload = sprintf('roi_%0.3d_inf_prev_cv.mat',rois(r));
    if exist(fullfile(rootdir,fnmload),'file')
        load(fullfile(rootdir,fnmload));
        realdata = ansMatAll(:,1);
        ...
        subplot(2,2,3);
        % compute pvals:
        pvals = calcPvalVoxelWise(ansMatAll);
        pvalsout(:,r) = pvals; 
        logpvals  = log10(pvals.*(-1));
        scatter(1:size(pvals,1),...
            logpvals,...
            40,[1 0 0],'o','filled','MarkerFaceAlpha',0.5);
        hold on;
        ...
        %% save figure
        suptitle(sprintf('roi %0.3d - label - %s # shuf - %0.3d',r, ROI{r}, size(ansMatAll,2) ));
...
save(fullfile(rootdir,'all_sub_data_from_100_shufs_cross_validate.mat'),...
    'percest');
return;
