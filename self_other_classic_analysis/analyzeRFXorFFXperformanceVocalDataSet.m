function analyzeRFXorFFXperformanceVocalDataSet()
fNamesToAnalyze = getFileNames('VocalDataSe*_75*27-slSize*.mat');
reportNaNs(fNamesToAnalyze)
...
cvFoldData.SigMap = fdr_bh(SigFDR,0.05,'pdep','no');
...
fid = fopen(fullfile(...
        rootFodler,'replicabilityAnalysis',...
        'ReplicabiliyAnalysisFFXorRFX_Paired_correctFFX-05.txt'),'w+');
...
subsToTest = setxor(1:218,cvFoldData.SubsUsd);
...
rawFile = 'RFXdatastats_normalized_sep_beta_.mat';
load(fullfile(rootDir,rawFile),'data','labels');
% load('RussBaseData.mat','data','labels');
rawData.subjects = 1:218;
rawData.deltaDat = data(labels==1,:) - data(labels==2,:);
end

function tabulateResults()
fnTosv = 'ReplicabiliyAnalysisFFXorRFX_Paired_correctFFX-05.txt';
rootFodler = ...
    'F:\vocalDataSet\processedData\matFilesProcessedData\vocalDataSetResults\FFXandRFX_files_readyForanalysis';

txtFn = fullfile(rootFodler,'replicabilityAnalysis',fnTosv);
[fp,~] = fileparts(txtFn);
if exist(txtFn,'file')
    t = readtable(txtFn);
    %% plot voxels passing:
