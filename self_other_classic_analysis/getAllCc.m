function [values, instances, rp] = getAllCc(Xin, conn, n)
...
%       array,  same number of dimensions as the input array. If empty
...
% Outputs:
% --------
%   values - cluster sizes 
%   instances - number of instances each cluster size exists in map 
%
...
% get the number of clusters in each size
allCc = [];
allClusters = [rp.Area];
values = unique(allClusters)';
instances = histc(allClusters,values)';
% 
% for s = 1:rp(1).Area
%     allCc(s,:) = sum([rp.Area]==s);
% end
