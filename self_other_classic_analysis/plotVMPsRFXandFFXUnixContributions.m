function plotVMPsRFXandFFXUnixContributions()
...
for z = 1:length(methodsUsed)
    for k = 1:length(uniqSig);
        for i = 1:length(subsOut)
            ...
            %% stuff for plotting one sphere for verifciation purposes 
            % to see voxel idx you ned to change type of vmp map to r map. 
%             rfxOnly = [17173 16274];
%             ffxOnly = [14636 13691];
%             commonV =[14174];%commonV;
            dataToPlot(rfxOnly) = 0;
            dataToPlot(ffxOnly) = 0;
            dataToPlot(commonV) = commonV;
            %%
%             dataToPlot(rfxOnly) = -5;
%             dataToPlot(ffxOnly) = 5;
%             dataToPlot(commonV) = 1;
            mapTitle = sprintf('%s %s %.2f RFX (%d) neg, FFX (%d) pos common (%d) # min pval %d',...
                subsOut{i},...
                methodsUsed{z},...
                uniqSig(k),...
                length(rfxOnly),...
